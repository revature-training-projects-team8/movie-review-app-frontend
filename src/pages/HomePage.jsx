...
const HomePage = () => {
  ...
  const loadMoviesFromDatabase = async () => {
    setLoading(true);
    try {
      // Load movies directly from database using authenticated API
      ...
      if (error.response?.status === 403) {
        ...
        console.error(
          "🔥 Backend server error - check your Spring Boot console for details"
        );
        console.error(
          "💡 Common causes: database connection, missing tables, backend code errors"
        );
      }
      setMovies([]);
      setFilteredMovies([]);
      ...
      <div className="bg-gradient-to-r from-blue-700 via-purple-700 to-pink-700 text-white py-20 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center">
            ...
            >
              <span className="text-white">Discover Amazing Movies</span>
            </h1>
            <p
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4">
              <div className="bg-white/30 backdrop-blur-md rounded-xl px-8 py-4 border-2 border-white/50 shadow-2xl">
                ...
                <div
                  className="text-sm text-white font-bold mt-1"
              ...
              <div className="bg-white/30 backdrop-blur-md rounded-xl px-8 py-4 border-2 border-white/50 shadow-2xl">
                ...
                >
                  {movies.length > 0
                    ? (
                        movies.reduce((sum, m) => sum + (m.avgRating || 0), 0) /
                        movies.length
                      ).toFixed(1)
                    : "N/A"}
                </div>
        ...
        <div className="bg-gray-800 rounded-xl shadow-xl p-6 mb-8 border border-gray-700">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center border-2 border-gray-600 bg-gray-700 rounded-lg focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all">
                <Search className="ml-4 text-gray-400 w-5 h-5 pointer-events-none flex-shrink-0" />
                <input
                  type="text"
                  placeholder="Search movies, genres, or directors..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="flex-1 px-3 py-3 bg-gray-700 outline-none text-white placeholder-gray-400"
                />
              </div>
            </div>
            ...
            <div className="relative md:w-56">
              <div className="flex items-center border-2 border-gray-600 bg-gray-700 rounded-lg focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500 transition-all">
                <Filter className="ml-4 text-gray-400 w-5 h-5 pointer-events-none flex-shrink-0" />
                <select
                  value={selectedGenre}
                  onChange={(e) => setSelectedGenre(e.target.value)}
                  className="flex-1 px-3 py-3 bg-gray-700 appearance-none cursor-pointer outline-none text-white pr-8"
                >
        ...
        {loading && (
          <div className="flex justify-center items-center py-12">
            ...
            <span className="ml-3 text-gray-300">
              Initializing movie database and loading movies...
            </span>
          </div>
        )}
